...
#error \
  "You must either define NUNAVUT_ASSERT or you need to disable assertions" \
...
static inline size_t nunavutChooseMin(const size_t a, const size_t b) { return (a < b) ? a : b; }
...
static inline size_t nunavutSaturateBufferFragmentBitLength(
  const size_t buffer_size_bytes, const size_t fragment_offset_bits,
...
static inline void nunavutCopyBits(
  void * const dst, const size_t dst_offset_bits, const size_t length_bits, const void * const src,
  const size_t src_offset_bits)
{
    ...
    (0U == (dst_offset_bits % 8U)))  // Aligned copy, optimized, most common case.
  {
    ...
    NUNAVUT_ASSERT(
      ((psrc < pdst)
         ? ((uintptr_t)(psrc + ((src_offset_bits + length_bits + 8U) / 8U)) <= (uintptr_t)pdst)
         : 1));
    NUNAVUT_ASSERT(
      ((psrc > pdst)
         ? ((uintptr_t)(pdst + ((dst_offset_bits + length_bits + 8U) / 8U)) <= (uintptr_t)psrc)
         : 1));
    while (last_bit > src_off) {
      ...
      const uint8_t in =
        (uint8_t)((uint8_t)(psrc[src_off / 8U] >> src_mod) << dst_mod) & 0xFFU;  // NOSONAR
      // Intentional violation of MISRA: indexing on a pointer.
      // This simplifies the implementation greatly and avoids pointer arithmetics.
...
static inline void nunavutGetBits(
  void * const output, const void * const buf, const size_t buf_size_bytes, const size_t off_bits,
  const size_t len_bits)
{
  NUNAVUT_ASSERT(output != NULL);
  NUNAVUT_ASSERT(buf != NULL);
  const size_t sat_bits =
    nunavutSaturateBufferFragmentBitLength(buf_size_bytes, off_bits, len_bits);
...
static inline int8_t nunavutSetBit(
  uint8_t * const buf, const size_t buf_size_bytes, const size_t off_bits, const bool value)
{
  NUNAVUT_ASSERT(buf != NULL);
...
static inline int8_t nunavutSetUxx(
  uint8_t * const buf, const size_t buf_size_bytes, const size_t off_bits, const uint64_t value,
  const uint8_t len_bits)
{
  ...
  const uint8_t tmp[sizeof(uint64_t)] = {
    (uint8_t)((value >> 0U) & 0xFFU),  (uint8_t)((value >> 8U) & 0xFFU),
...
static inline int8_t nunavutSetIxx(
  uint8_t * const buf, const size_t buf_size_bytes, const size_t off_bits, const int64_t value,
  const uint8_t len_bits)
{
...
static inline uint8_t nunavutGetU8(
  const uint8_t * const buf, const size_t buf_size_bytes, const size_t off_bits,
  const uint8_t len_bits);
...
{
  NUNAVUT_ASSERT(buf != NULL);
  const size_t bits = nunavutSaturateBufferFragmentBitLength(
    buf_size_bytes, off_bits, nunavutChooseMin(len_bits, 8U));
...
static inline uint16_t nunavutGetU16(
  const uint8_t * const buf, const size_t buf_size_bytes, const size_t off_bits,
  const uint8_t len_bits)
{
  NUNAVUT_ASSERT(buf != NULL);
  const size_t bits = nunavutSaturateBufferFragmentBitLength(
    buf_size_bytes, off_bits, nunavutChooseMin(len_bits, 16U));
...
static inline uint32_t nunavutGetU32(
  const uint8_t * const buf, const size_t buf_size_bytes, const size_t off_bits,
  const uint8_t len_bits)
{
  NUNAVUT_ASSERT(buf != NULL);
  const size_t bits = nunavutSaturateBufferFragmentBitLength(
    buf_size_bytes, off_bits, nunavutChooseMin(len_bits, 32U));
  NUNAVUT_ASSERT(bits <= (sizeof(uint32_t) * 8U));
  uint8_t tmp[sizeof(uint32_t)] = {0};
  nunavutCopyBits(&tmp[0], 0U, bits, buf, off_bits);
  return (
    uint32_t)(tmp[0] | ((uint32_t)tmp[1] << 8U) | ((uint32_t)tmp[2] << 16U) | ((uint32_t)tmp[3] << 24U));
}

static inline uint64_t nunavutGetU64(
  const uint8_t * const buf, const size_t buf_size_bytes, const size_t off_bits,
  const uint8_t len_bits)
{
  NUNAVUT_ASSERT(buf != NULL);
  const size_t bits = nunavutSaturateBufferFragmentBitLength(
    buf_size_bytes, off_bits, nunavutChooseMin(len_bits, 64U));
  NUNAVUT_ASSERT(bits <= (sizeof(uint64_t) * 8U));
  uint8_t tmp[sizeof(uint64_t)] = {0};
  nunavutCopyBits(&tmp[0], 0U, bits, buf, off_bits);
  return (
    uint64_t)(tmp[0] | ((uint64_t)tmp[1] << 8U) | ((uint64_t)tmp[2] << 16U) | ((uint64_t)tmp[3] << 24U) | ((uint64_t)tmp[4] << 32U) | ((uint64_t)tmp[5] << 40U) | ((uint64_t)tmp[6] << 48U) | ((uint64_t)tmp[7] << 56U));
}

static inline int8_t nunavutGetI8(
  const uint8_t * const buf, const size_t buf_size_bytes, const size_t off_bits,
  const uint8_t len_bits)
{
...
static inline int16_t nunavutGetI16(
  const uint8_t * const buf, const size_t buf_size_bytes, const size_t off_bits,
  const uint8_t len_bits)
{
...
static inline int32_t nunavutGetI32(
  const uint8_t * const buf, const size_t buf_size_bytes, const size_t off_bits,
  const uint8_t len_bits)
{
...
static inline int64_t nunavutGetI64(
  const uint8_t * const buf, const size_t buf_size_bytes, const size_t off_bits,
  const uint8_t len_bits)
{
...
static inline int8_t nunavutSetF16(
  uint8_t * const buf, const size_t buf_size_bytes, const size_t off_bits, const float value)
{
...
static_assert(
  NUNAVUT_PLATFORM_IEEE754_DOUBLE,
...
static inline int8_t nunavutSetF64(
  uint8_t * const buf, const size_t buf_size_bytes, const size_t off_bits, const double value)
{
  // Intentional violation of MISRA: use union to perform fast conversion from an IEEE 754-compatible native
  // representation into a serializable integer. The assumptions about the target platform properties are made
  // clear. In the future we may add a more generic conversion that is platform-invariant.
